...
export default function MyKeyboard() {
  ...
  const firstNumberDisplay = () => {
    if (solution !== null) {
        return (
        <Text style={solution < 99999 
          ? [Styles.screenFirstNum, {color: globalColors.solution}] 
          : [Styles.screenFirstNum, {fontSize: 30, color: globalColors.solution}]}
        >
          {solution?.toString()}
        </Text>); 
    }
    ...
    if (firstNumber === "") {
      return <Text style={Styles.screenFirstNum}>{""}</Text>;
    }
    ...
    if (firstNumber.length > 5 && firstNumber.length < 8) {
      return (
        <Text style={[Styles.screenFirstNum, { fontSize: 33 }]}>
          {firstNumber}
        </Text>
      );
    }
    if (firstNumber.length > 7) {
      return (
        <Text style={[Styles.screenFirstNum, { fontSize: 30 }]}>
          {firstNumber}
        </Text>
      );
    }
